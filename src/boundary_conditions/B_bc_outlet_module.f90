...
implicit none
!=======================================================================
!============== OUTLET BOUNDARY CONDITION DATA STRUCTURE ===============
!=======================================================================
type, extends(bc_base_t), public :: bc_outlet_t
	real(8) :: outlet_pressure = 100000.d0
	
contains
	procedure :: apply_patch => apply_outlet
	procedure :: ADdiff_apply_patch => ADdiff_apply_outlet
	procedure :: apply_ghosts_patch => apply_outlet_ghosts
	procedure :: ADdiff_apply_ghosts_patch => ADdiff_apply_outlet_ghosts
	procedure :: apply_jmatrix_inv_patch => apply_outlet_jmatrix_inv
	procedure :: apply_jmatrix_visc_patch => apply_outlet_jmatrix_visc
end type

contains
!=======================================================================
!================== OUTLET BOUNDARY CONDITION METHODS ==================
!=======================================================================
    subroutine apply_outlet(this, R_gas, k, values_ptr, values_grad_ptr, name, update_grad)
		implicit none
		class(bc_outlet_t), intent(inout) :: this
        ...
        case('Vel')														!=== VELOCITY FIELD ===
			call apply_extrapolation0_loop(dim, ncells, this%nfaces, 1, dim, update_grad,&
        ...
        case('Prs')														!=== PRESSURE FIELD ===
			...
			case(BC_OUTLET_SUBSONIC)
				fxd_values = this%outlet_pressure
        ...
        case('Q')														!=== PRIMITIVE VARIABLES [P U V W T] FIELD ===
			...
			case(BC_OUTLET_SUBSONIC)
				fxd_values = this%outlet_pressure
				call apply_subsonic_outlet_Q_loop(dim, ncells, this%nfaces, update_grad,&
												  fxd_values, values_ptr, values_grad_ptr,&
												  this%b_indices, this%cell_indices, this%face_indices,&
												  this%mesh%cell_center, this%mesh%face_center, this%mesh%face_normal)
											 
			case(BC_OUTLET_SUPERSONIC)
				call apply_extrapolation0_loop(dim, ncells, this%nfaces, 1, dim+2, update_grad,&
											   values_ptr, values_grad_ptr,&
											   this%b_indices, this%cell_indices, this%face_indices)
	...
	subroutine apply_outlet_ghosts(this, values_ptr, values_grad_ptr, name, update_grad)
		implicit none
		class(bc_outlet_t), intent(inout) :: this
		real(8), intent(inout), contiguous :: values_ptr(:, :)
		real(8), intent(inout), contiguous :: values_grad_ptr(:, :)
        ...
        select case(trim(name))
        case('Vel')														!=== VELOCITY FIELD ===
			call apply_extrapolation0_ghost_loop(dim, ncells, this%nfaces, 1, dim, update_grad,&
											     values_ptr, values_grad_ptr,&
											     this%b_indices, this%cell_indices, this%face_indices)
        ...
        case('Prs')														!=== PRESSURE FIELD ===
			select case (this%bc_subtype)	
			case(BC_OUTLET_SUBSONIC)
				fxd_values(1) = this%outlet_pressure
        ...
        case('Q')														!=== PRIMITIVE VARIABLES [P U V W T] FIELD ===
			...
			case(BC_OUTLET_SUBSONIC)
				fxd_values(1) = this%outlet_pressure
				call apply_subsonic_outlet_Q_ghost_loop(dim, ncells, this%nfaces, update_grad,&
													    fxd_values, values_ptr, values_grad_ptr,&
													    this%b_indices, this%cell_indices, this%face_indices,&
													    this%mesh%cell_center, this%mesh%face_center, this%mesh%face_normal)
												  
			case(BC_OUTLET_SUPERSONIC)
				call apply_extrapolation0_ghost_loop(dim, ncells, this%nfaces, 1, dim+2, update_grad,&
												     values_ptr, values_grad_ptr,&
												     this%b_indices, this%cell_indices, this%face_indices)
			end select
        case default
	...
	subroutine apply_outlet_jmatrix_inv(this, R_gas, k, values_ptr, values, map_LB, name)
		implicit none
		class(bc_outlet_t), intent(inout) :: this
		real(8), intent(in) :: R_gas, k
	...
	subroutine apply_outlet_jmatrix_visc(this, R_gas, k, cp, Pr, values_ptr, values, map_LB, name)
		implicit none
		class(bc_outlet_t), intent(inout) :: this
	...
	SUBROUTINE ADdiff_apply_outlet(this, R_gas, k, values_ptr, values_ptrd,&
		...
		SUBROUTINE Q_FXD_PRS_D()
			CALL APPLY_FIXED_VALUE_BC_D(dim, n, values_ptrd(1, cell_idx),&
			&                           values_grad_ptrd(1:dim, cell_idx),&
			values_grad_ptrd(1:dim, ncells+b_idx), dist, update_grad)
		END SUBROUTINE

		!  Differentiation of q_extplt_vel in forward (tangent) mode:
		!   variations   of useful results: values_grad_ptr values_ptr
		!   with respect to varying inputs: values_grad_ptr values_ptr
		SUBROUTINE Q_EXTPLT_VEL_D()
			DO d=1,dim
				off = dim*d
				CALL APPLY_EXTRAPOLATION0_BC_D(dim, values_ptrd(1+d, cell_idx), &
		...
		!  Differentiation of q_extplt_temp in forward (tangent) mode:
		!   variations   of useful results: values_grad_ptr values_ptr
		!   with respect to varying inputs: values_grad_ptr values_ptr
		SUBROUTINE Q_EXTPLT_TEMP_D()
			off = dim*(dim+1)
			CALL APPLY_EXTRAPOLATION0_BC_D(dim, values_ptrd(dim+2, cell_idx), &
			&                              values_grad_ptrd(1+off:dim+off, cell_idx),&
			   values_ptrd(dim+2, ncells+b_idx), &
			&                              values_grad_ptrd(1+off:dim+off, &
			&                              ncells+b_idx), update_grad)
		END SUBROUTINE
        
    END SUBROUTINE
	
	SUBROUTINE ADdiff_apply_outlet_ghosts(this, values_ptr, values_ptrd,&
										  values_grad_ptr, values_grad_ptrd, name, update_grad)
		IMPLICIT NONE
		CLASS(bc_outlet_t), INTENT(INOUT) :: this
		REAL(8), INTENT(IN), CONTIGUOUS :: values_ptr(:, :)
		...
		!   with respect to varying inputs: values_grad_ptr values_ptr
		SUBROUTINE Q_FXD_PRS_D()
			CALL APPLY_FIXED_VALUE_GHOST_BC_D(dim, n, values_ptrd(1, cell_idx),&
			&                           values_grad_ptrd(1:dim, cell_idx),&
			values_ptrd(1, ncells+b_idx),&
			values_grad_ptrd(1:dim, ncells+b_idx), dist, update_grad)
		END SUBROUTINE
		...
		SUBROUTINE Q_EXTPLT_VEL_D()
			DO d=1,dim
				off = dim*d
				CALL APPLY_EXTRAPOLATION0_GHOST_BC_D(dim, values_ptrd(1+d, cell_idx), &
				&                                values_grad_ptrd(1+off:dim+off, cell_idx),&
				&                                values_ptrd(1+d, ncells+b_idx), &
				&                                values_grad_ptrd(1+off:dim+off, ncells+b_idx), update_grad)
			END DO
