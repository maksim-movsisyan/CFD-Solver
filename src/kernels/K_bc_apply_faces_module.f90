module bc_apply_faces_module
use bc_kernels_module
implicit none
contains
!=======================================================================
!====================== FIXED VALUE LOOP SUBROUTINE ====================
!=======================================================================
pure subroutine apply_fixed_value_loop(dim, ncells, nfaces, start_vidx, end_vidx, update_grad,&
	...
	do i = 1, nfaces					
		...
		do d = start_vidx, end_vidx
			off = dim*(d-1)
			call apply_fixed_value_bc(dim, n,&
...
!=======================================================================
pure subroutine apply_extrapolation0_loop(dim, ncells, nfaces, start_vidx, end_vidx, update_grad,&
	...
	do i = 1, nfaces	
		...
		do d = start_vidx, end_vidx
			off = dim*(d-1)
			call apply_extrapolation0_bc(dim,&
								 values_ptr(d, cell_idx), values_grad_ptr(1+off:dim+off, cell_idx),&
...
!=======================================================================
pure subroutine apply_fixed_gradient_loop(dim, ncells, nfaces, start_vidx, end_vidx, update_grad,&
	...
	do i = 1, nfaces					
		...
		do d = start_vidx, end_vidx
			off = dim*(d-1)
			call apply_fixed_gradient_bc(dim, n,&
										 values_ptr(d, cell_idx), values_grad_ptr(1+off:dim+off, cell_idx),&
										 fixed_ngrads(d),&
										 values_ptr(d, ncells+b_idx), values_grad_ptr(1+off:dim+off, ncells+b_idx),&
										 dist, update_grad)
...
!=======================================================================
pure subroutine apply_slip_vector_loop(dim, ncells, nfaces, start_vidx, end_vidx, update_grad,&
									   values_ptr, values_grad_ptr,&
									   b_indices, cell_indices, face_indices,&
									   cell_center, face_center, face_normal)
	implicit none
	...
	do i = 1, nfaces
		...
		call apply_slip_vector_bc(dim, n,&
...
!=======================================================================
pure subroutine apply_subsonic_inlet_Q_loop(dim, ncells, nfaces, update_grad,&
											fixed_values, values_ptr, values_grad_ptr,&
											b_indices, cell_indices, face_indices,&
											cell_center, face_center, face_normal)
	implicit none
...
pure subroutine apply_subsonic_outlet_Q_loop(dim, ncells, nfaces, update_grad,&
											 fixed_values, values_ptr, values_grad_ptr,&
											 b_indices, cell_indices, face_indices,&
											 cell_center, face_center, face_normal)
	implicit none
...
pure subroutine apply_symm_Q_loop(dim, ncells, nfaces, update_grad,&
								  values_ptr, values_grad_ptr,&
								  b_indices, cell_indices, face_indices,&
						   		  cell_center, face_center, face_normal)
	implicit none
...
pure subroutine apply_isothermal_noslip_wall_Q_loop(dim, ncells, nfaces, update_grad,&
													fixed_values, values_ptr, values_grad_ptr,&
													b_indices, cell_indices, face_indices,&
													cell_center, face_center, face_normal)
	implicit none
...
pure subroutine apply_fixed_flux_noslip_wall_Q_loop(dim, ncells, nfaces, update_grad,&
													fixed_values, values_ptr, values_grad_ptr,&
													b_indices, cell_indices, face_indices,&
													cell_center, face_center, face_normal)
	implicit none
...
pure subroutine apply_reiman_slip_wall_Q_loop(dim, ncells, nfaces, update_grad, k, R_gas,&
											  values_ptr, values_grad_ptr,&
											  b_indices, cell_indices, face_indices,&
											  cell_center, face_center, face_normal)
	implicit none
	...
	do i = 1, nfaces					
		...
		!pressure:
		call apply_reiman_pressure_bc(dim,&
									  values_ptr(1, cell_idx), values_grad_ptr(1:dim, cell_idx),&
									  values_ptr(2:dim+1, cell_idx), values_ptr(dim+2, cell_idx),&
									  n, k, R_gas,&
									  values_ptr(1, ncells+b_idx), values_grad_ptr(1:dim, ncells+b_idx),&
									  update_grad)
